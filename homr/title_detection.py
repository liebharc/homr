...
# Globals
_reader: torchfree_ocr.Reader | None = None  # type: ignore
    ...
    if _reader is None:
        with _reader_lock:
            if _reader is None:  # double-checked locking
                _reader = torchfree_ocr.Reader(["de", "en"])
...
def _detect_title_task(debug: Debug, top_staff: Staff) -> str:
    ...
    ocr_results = _reader.readtext(ocr_input, detail=1, paragraph=True)
    if not ocr_results:
        return ""

    # Each result is (bbox, text, confidence)
    # Pick the text with the largest bbox area
    def bbox_area(bbox: list[list[int]]) -> int:
        # bbox = [[x1, y1], [x2, y2], [x3, y3], [x4, y4]]
        xs = [p[0] for p in bbox]
        ys = [p[1] for p in bbox]
        return (max(xs) - min(xs)) * (max(ys) - min(ys))

    largest_text = max(ocr_results, key=lambda r: bbox_area(r[0]))[1]  # get text of largest area
